...
def test_cross_tenant_is_404_not_403(client: TestClient) -> None:
...
def test_forged_tenant_id_in_body_is_ignored(client: TestClient) -> None:
    tenant_a = uuid4()
    tenant_b = uuid4()
    headers_a = auth_headers(tenant_id=tenant_a)
    created = client.post(
        "/api/v1/projects/",
...
def test_same_name_allowed_across_tenants(client: TestClient) -> None:
    name = f"project-{uuid4()}"
    first = client.post(
...
def test_same_tenant_duplicate_name_is_still_409(client: TestClient) -> None:
